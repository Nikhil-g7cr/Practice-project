...
const Cart: React.FC = () => {
  ...
  const { cartItems, totalQuantity, totalPrice } = useAppSelector((state) => state.cart);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 animate-fade-in">
        ...
        <p className="text-slate-500 mb-8 text-center max-w-md">
          Looks like you haven't added anything to your cart yet. Browse our top products and find something you love!
        </p>
        <button
          onClick={() => navigate('/')}
          className="px-8 py-3 bg-cyan-600 text-white font-semibold rounded-xl hover:bg-cyan-700 transition-all shadow-lg shadow-cyan-200 hover:-translate-y-0.5"
        >
          Continue Shopping
        </button>
      </div>
  ...
  // Populated Cart View
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 md:py-16 mt-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cartItems.map((item) => (
            <div key={item._id} className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-100 relative group">
              
              {/* Product Image */}
              <div className="w-full sm:w-28 h-28 bg-slate-50 rounded-xl overflow-hidden flex items-center justify-center flex-shrink-0">
                {item.imageUrl ? (
                  <img src={item.imageUrl} alt={item.name} className="object-contain w-full h-full p-2 hover:scale-110 transition-transform duration-300" />
                ) : (
              ...
              <div className="flex-grow text-center sm:text-left">
                ...
                <p className="text-cyan-600 font-bold text-lg mt-1">${item.price.toFixed(2)}</p>
              </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 h-fit lg:sticky lg:top-24">
          ...
          <div className="space-y-4 text-slate-600 mb-8">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-emerald-500 font-semibold bg-emerald-50 px-2 py-0.5 rounded text-sm tracking-wide uppercase">Free</span>
            </div>
            <div className="flex justify-between items-center">
          ...
          <div className="border-t border-slate-100 pt-6 mb-8 flex justify-between items-end">
            ...
            <span className="text-3xl font-bold text-cyan-600">${totalPrice.toFixed(2)}</span>
          </div>
